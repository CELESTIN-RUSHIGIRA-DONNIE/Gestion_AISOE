                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="page-sub-header">
                                <h3 class="page-title">Événement</h3>
                                <ul class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="liste_evenement.php">Liste des Événements</a></li>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title">Contenus de l'événement</h5>
                            </div>
                            <div class="card-body">
                                <?php
                                if (isset($_GET['id'])) {
                                    $evenement_id = $_GET['id'];
                                    $evenement = "SELECT * FROM evenement WHERE id ='$evenement_id'";
                                    $evenement_run = mysqli_query($con, $evenement);

                                    if (mysqli_num_rows($evenement_run) > 0) {
                                        $evenement_row = mysqli_fetch_array($evenement_run)
                                ?>
                                        <form action="code.php" method="POST" enctype="multipart/form-data">
                                            <input type="hidden" name="evenement_id" value="<?= $evenement_row['id']; ?>">
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <label>Titre :</label>
                                                        <input type="text" b value="<?= $evenement_row['titre'] ?>" name="titre" class="form-control">
                                                    </div>

                                                    <div class="form-group">
                                                        <label>Description : </label>
                                                        <textarea rows="3" cols="3" class="form-control" name="description" placeholder="Enter description"><?= $evenement_row['description'] ?></textarea>
                                                    </div>

                                                    <div class="form-group">
                                                        <label>Contenu : </label>
                                                        <textarea rows="5" cols="5" class="form-control" name="contenu" placeholder="Enter message"><?= $evenement_row['detail'] ?></textarea>
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Image :</label>
                                                        <input type="hidden" name="old_photo" value="<?= $evenement_row['photo']; ?>" class="form-control">
                                                        <input type="file" name="new_photo" class="form-control">
                                                    </div>
                                                    <div class="form-group">
                                                        <label>Visible : </label>
                                                        <input type="checkbox" name="visible" <?= $evenement_row['statut'] == 1 ? 'checked' : '' ?> width="70px" height="70px">
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="text-end">
                                                <button type="submit" name="edit_evenement" class="btn btn-primary">Modifier</button>
                                            </div>
                                        </form>
                                <?php
